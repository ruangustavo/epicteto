...
export async function composeUp(cfg: RunConfig, worktree: string, stateDir: string): Promise<ComposeStack | null> {
  const file = composeFile(worktree);
  if (!file) return null;
  const project = `agent-${cfg.name}-${cfg.issueNumber}`.toLowerCase();
  const services = (await $`docker compose -f ${file} config --services`.text()).split("\n").filter(Boolean);
  ...
  return { project, network: `${project}_default` };
}
