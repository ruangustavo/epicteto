...
const installRes = await fetch(`https://api.github.com/${process.env.ORG ? `orgs/${process.env.ORG}/installation` : `repos/${process.env.REPO}/installation`}`, { headers });
...
const tokenRes = await fetch(`https://api.github.com/app/installations/${id}/access_tokens`, { method: "POST", headers });
