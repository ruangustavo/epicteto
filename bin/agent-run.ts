...
async function runAgentAndChecks(
  prompt: string,
...
async function preExisting(check: CheckResult): Promise<boolean> {
  ...
  const baselineMounts: ContainerMounts = {
    ...mounts,
    worktree: p.baselineWorktree,
  };
 ...
 */
async function checksWithRetry(
  resultFile: string,
  prUrl?: string,
): Promise<{
  green: boolean;
  preExisting: CheckResult | null;
}> {
  for (let attempt = 1; ; attempt++) {
    await gh.upsertStatusComment(
      cfg,
      statusComment({
        phase: `running: checks (attempt ${attempt}/${MAX_ATTEMPTS})`,
        runUrl,
        checks,
        prUrl,
      }),
...
async function verifyUi(issue: gh.Issue, files: string[], prUrl: string): Promise<Verification> {
  ...
  if (!cfg.attachToken)
    return {
      ...none,
      failure: "UI changed but ATTACH_TOKEN is not configured; no video.",
    };

  await gh.upsertStatusComment(
    cfg,
    statusComment({
      phase: "running: recording verification",
      runUrl,
      checks,
      prUrl,
    }),
  ...
  const mountsNet: ContainerMounts = {
    ...mounts,
  ...
  try {
    if (!existsSync(`${p.state}/boot.sh`) || !existsSync(`${p.state}/verify.sh`)) {
      const prompt = await renderTemplate("verify-prompt.md", {
    ...
    if (rec.exitCode !== 0) {
      ...
      const prompt = await renderTemplate("verify-repair-prompt.md", {
        exit_code: String(rec.exitCode),
        output: rec.output.slice(-4000),
        files: fileList,
      });
...
async function implement(cfg: RunConfig) {
  ...
  if (outcome === "no-result")
    return finish("needs input: agent produced no result file", "agent:needs-input");
  ...
  if (!(await git.hasCommitsAheadOfBase(p.worktree, cfg.baseBranch)))
    return finish("needs input: agent made no changes", "agent:needs-input");
  ...
  if (green) {
    const verification = await verifyUi(
      issue,
      await git.changedFiles(p.worktree, cfg.baseBranch),
      prUrl,
    );
    const section = verificationMarkdown(verification);

    if (section)
      body = body.replace(`Closes #${issue.number}`, `${section}\n\nCloses #${issue.number}`);
  }
  ...
  if (outcome2.preExisting)
    return finish(
      `in review (\`${outcome2.preExisting.name}\` already red on ${cfg.baseBranch})`,
      "agent:in-review",
      { prUrl },
    );

  return finish(
    `needs input: checks still failing after ${MAX_ATTEMPTS} attempts, PR opened as draft`,
    "agent:needs-input",
    { prUrl },
  );
}
...
async function reviewRound(cfg: RunConfig, prNumber: number, reviewBody: string) {
  ...
  await gh.upsertStatusComment(
    cfg,
    statusComment({
      phase: "running: addressing review",
      runUrl,
      checks,
      prUrl,
    }),
  ...
  const prompt = await review.renderReviewPrompt(cfg, {
    pr,
  ...
  if (outcome === "quota")
    return finish("paused: Codex quota exhausted", "agent:quota-paused", { prUrl });

  if (outcome === "no-result")
    return finish("needs input: agent produced no review result", "agent:needs-input", { prUrl });

  const result = review.parseReviewResult(outcome.resultText ?? "");

  if (!result)
    return finish("needs input: review result is malformed", "agent:needs-input", {
      prUrl,
      detail: agentOutputDetails(outcome.output),
    });
  ...
  return green
    ...
    : finish(
        `needs input: checks still failing after ${MAX_ATTEMPTS} attempts`,
        "agent:needs-input",
        { prUrl },
      );
}
...
async function rerecord(cfg: RunConfig, prNumber: number) {
  ...
  if (verification.url)
    await review.commentOnPr(
      cfg,
      prNumber,
      `Re-recorded the verification video:\n\n${verification.url}`,
    );

  return finish(
    verification.url ? "in review" : "needs input: re-record failed",
    verification.url ? "agent:in-review" : "agent:needs-input",
    {
      prUrl,
